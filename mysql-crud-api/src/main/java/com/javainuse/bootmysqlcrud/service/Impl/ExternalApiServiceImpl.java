...
public class ExternalApiServiceImpl implements ExternalApiService {
    ...
    public ExternalSunatDto consultarSunat(String ruc) {
        String url = "https://api.decolecta.com/v1/sunat/ruc?numero=" + ruc;
        ExternalSunatDto externalReniecDto = (ExternalSunatDto)this.webClient.get().uri(url, new Object[0]).header("Content-Type", new String[] { "application/json" }).header("Authorization", new String[] { "Bearer sk_9344.ZETheeFkUTXUFM8xHI7dVBmwHvztm9fH" }).retrieve().bodyToMono(ExternalSunatDto.class).block();
        return externalReniecDto;
    }
}
